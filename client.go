...
func Dial(urlStr string, requestHeader http.Header) (*Client, error) {
	...
	if err != nil {
		log.Printf("[wnet-debug]: websocket dial err: %v", err)
		return nil, err
	}
...
func (c *Client) Close() {
	err := c.conn.Close()
	if err != nil {
		log.Printf("[wnet-debug]: client close err: %v", err)
	}
	c.exitChan <- true
	close(c.exitChan)

	log.Printf("[wnet-debug]: client closed")
}
...
func (c *Client) readMessages() {
	...
	defer log.Printf("[wnet-debug]: goroutine readMessages closed")
	for {
		var resp Response
...
func (c *Client) handleResponse(resp Response) {
	if resp.ID == 0 {
		...
		if h, ok := c.handlers[resp.Method]; ok {
			...
			log.Printf("[wnet-debug] ignoring response %s with no handler", resp.Method)
		}
		c.mu.Unlock()
	} else {
		id := resp.ID
		...
		switch {
		case call == nil:
			log.Printf("[wnet-debug] ignoring response %d with no corresponding request", id)
		case resp.Error != nil:
...
// 发起请求，阻塞到数据返回或超时
func (c *Client) Call(method string, params, reply interface{}) error {
	req := Request{
		Method: method,
	}
	...
	t := time.NewTimer(time.Millisecond * time.Duration(c.timeout))
	select {
